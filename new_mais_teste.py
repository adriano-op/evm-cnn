...
# folder_path = '/content/drive/MyDrive/mestrado/new_remotePPG/signal/'
# processed_data_path = '/content/drive/MyDrive/mestrado/new_remotePPG/saida/'
...
num_classes = 43 #109               # Total number of classes (individuals)
...
band_pass_4 = [0.01, 29]       # Four filter option,  10~30Hz
...
if(not _datagen):
    ...
    if(not _noimode):
        print('not _noimode',not _noimode)
    ...
    if(not _novmode):
        print('not _novmode',not _novmode)
...
# Using Data Generators
    ...
    # Running the model in Identification Mode
    if(not _noimode):
        print('not _noimode_2',not _noimode)
        # Evaluate the model to see the accuracy
        if(model is None):
            model = models.create_model_causal(window_size, num_channels, num_classes) ##
            model.summary()
        ...
        (loss, accuracy) = model.evaluate(x_test, y_test, verbose = 0)
        print('loss x_test ={:.4f}, accuracy: {:.4f}%\n'.format(loss,accuracy * 100))
    ...
    # Running the model in Verification Mode
    if(not _novmode):
        print('not _novmode_2',not _novmode)
        # Removing the last layers of the model and getting the features array
